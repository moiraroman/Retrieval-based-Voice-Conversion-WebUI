    ...
    class GUI:
        ...
        def launcher(self):
            ...
            layout = [
                [
                    sg.Frame(
                        title=i18n("Model Quikly Change"),
                        layout=[
                            [
                                sg.Text(i18n("Model List")),
                                sg.Combo(
                                    "",
                                    key="sg_ModelList",
                                    default_value="",
                                ),
                                sg.Button(i18n("ModelChange"), key="start_ModelChange"),
                            ],
                        ],
                    )
                ],
                    ...
                    sg.Frame(
                        ...
                        layout=[
                            ...
                            [
                                ...
                                sg.Button(
                                    i18n("Save Model Path"), key="start_ModelSave"
                                ),
            ...
            file_path = os.path.join(os.getcwd(), "ModeSave.csv")
            ...
            if not os.path.exists(file_path):
                ...
                with open(file_path, mode="w", newline="") as file:
                    ...
                    writer.writerow(["name", "pth_path", "index_path"])
            ModeSave_csv = []
            with open(file_path, mode="r", newline="") as file:
                ...
                for row in reader:
                    if row[0]:  # 只读取第一列非空的行
                        ModeSave_csv.append(row)

            list_values = [f"{row[1]}@{row[2]}" for row in ModeSave_csv]
            if list_values:  # 确保 list_values 不是空列表
                self.window["sg_ModelList"].update(
                    values=list_values, value=list_values[0]
                )       
        ...
        def event_handler(self):
            ...
            while True:
                ...
                """保存模型的行为"""
                if event == "start_ModelSave":
                    pth_path_t = values["pth_path"]
                    index_path_t = values["index_path"]
                    file_path = os.path.join(os.getcwd(), "ModeSave.csv")

                    new_option = pth_path_t + "@" + index_path_t
                    current_values = self.window["sg_ModelList"].Values

                    # 向选项列表添加新选项
                    if new_option not in current_values:
                        current_values.append(new_option)

                        # 更新 Combo，设置新选项并设为默认值
                        self.window["sg_ModelList"].update(
                            values=current_values, value=new_option
                        )
                        # 检查文件是否存在
                        if not os.path.exists(file_path):
                            # 如果文件不存在，则新建文件并写入列名称
                ...
                """切换模型的行为"""
                if event == "start_ModelChange":
                    selected_value = values["sg_ModelList"]
                    if selected_value:
                        parts = selected_value.split("@")

                        self.window["pth_path"].update(value=parts[0])
                        self.window["index_path"].update(value=parts[1])                    
